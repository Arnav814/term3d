...
dvec3 canvasToViewport(const SextantCoord coord, const SextantCoord canvasSize) {
	return dvec3{coord.x * (viewportWidth / canvasSize.x),
	             coord.y * (viewportHeight / canvasSize.y), viewportDistance};
}
...
std::pair<double, double> intersectRaySphere(const dvec3 origin, const dvec3 ray,
                                             const Sphere sphere) {
	...
	return std::make_pair((-b + sqrt(discriminant)) / (2.0 * a),
...
SphereDist closestIntersection(const Scene& scene, const dvec3 origin, const dvec3 direction,
...
static double computeLighting(const Scene& scene, const dvec3 point, const dvec3 normal,
	...
	for (const std::shared_ptr<const Light> light : scene.lights) {
		...
		auto sphereDist = closestIntersection(
		    scene, point, lightDir, 0.001,
		    tMax); // can't use numeric_limits::min() because of floating point errors
		...
		if (specular != -1) {
			...
			if (reflectedDotExit > 0) {
				intensity +=
				    light->getIntensity()
				    * pow(reflectedDotExit / (glm::length(reflected) * glm::length(exitVec)),
				          specular);
			}
		}
	}
...
Color traceRay(const Scene& scene, const dvec3 origin, const dvec3 direction, const double tMin,
               const double tMax, const uint recursionLimit) {
	auto closest = closestIntersection(scene, origin, direction, tMin, tMax);

	if (closest.closestT == std::numeric_limits<double>::infinity()) return BACKGROUND_COLOR;
	...
	localColor.color *=
	    computeLighting(scene, intersection, normal, -direction, closest.sphere.specular);
	...
	Color reflectedColor =
	    traceRay(scene, intersection, reflected, 0.001, tMax, recursionLimit - 1);

	// add them together
	RGBA combinedColor = localColor.color * (1.0 - closest.sphere.reflective)
	                     + reflectedColor.color * closest.sphere.reflective;
...
void rayRenderLoop(WindowedDrawing& rawCanvas, const bool& exit_requested,
	...
	const static Scene scene{
	    {
         Sphere(dvec3{0.0, -1.0, 3.0}, 1.0, Color{Category{true, 9}, RGBA{255, 0, 0, 255}}, 500,
         0.2),
         Sphere(dvec3{2.0, 0.0, 4.0}, 1.0, Color{Category{true, 10}, RGBA{0, 0, 255, 255}}, 500,
         0.3),
         Sphere(dvec3{-2.0, 0.0, 4.0}, 1.0, Color{Category{true, 11}, RGBA{0, 255, 0, 255}}, 10,
         0.4),
         Sphere(dvec3{0.0, -5001.0, 0.0}, 5000.0,
         Color{Category{true, 12}, RGBA{255, 255, 0, 255}}, 1'000, 0.5),
	     },
	    0.2,
	    {
	...
	while (not exit_requested) {
		...
		for (int x = -canvas.getWidth() / 2; x < canvas.getWidth() / 2; x++) {
			for (int y = -canvas.getHeight() / 2; y < canvas.getHeight() / 2; y++) {
				dvec3 direction =
				    cameraRotation * canvasToViewport(SextantCoord(y, x), canvas.getSize());
