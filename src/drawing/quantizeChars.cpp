...
getTrimmedColors(const charArray<Color>& arrayChar) {
	...
	*/
	...
	if (rankedCategories.size() == 1 && not rankedCategories.at(0).allowMixing) {
		...
		finalColors.first =
		    averageColor(applyAlphas(filterColorsByCategory(rankedCategories.at(0), flattened)));
		finalColors.second =
		    averageColor(applyAlphas(filterColorsByCategory(rankedCategories.at(1), flattened)));
